		...
		if tamano_actual_listasolicitudes < tamano_max_listasolicitudes:
			...
			if srcip == ipServidor:
				...
				if srcip in self.ListaSolicitudes:
					...
					if srcip in self.ListaAtacantes:
						...
						if srcip in self.ListaClientes:
							enviar.enviar_paquete(pkt,network,IpPuerto[dstip])									

						else:	
							#Debido a que no estaba en al lista de clientes lejitimos, se procede a añadirla
							self.ListaSolicitudes.append(srcip)
							#Se verifica que efectivamente  se haya añadido a la lista 
